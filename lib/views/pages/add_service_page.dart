...
class _AddServicePageState extends State<AddServicePage> {
  ...
  Future<String?> _uploadImageToStorage() async {
    ...
    try {
      ...
      final ext =
          (_imageMimeType ?? 'image/jpeg').split('/').last.replaceAll('jpeg', 'jpg');
      final fileName =
          'service_${DateTime.now().millisecondsSinceEpoch}.$ext';

      await supabase.storage.from('services').uploadBinary(
            fileName,
            _imageBytes!,
  ...
  Widget _buildPricingSection() {
    ...
    final packageName = _currentPackageKey[0].toUpperCase() +
        _currentPackageKey.substring(1);
